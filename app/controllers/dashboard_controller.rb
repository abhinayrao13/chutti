class DashboardController < ApplicationController
  ...
  def index
    ...
    @desc_checkins = [@desc_checkins.as_json.each{|x| x["type"] = "check_in"} + @desc_checkins.select{|x| x.check_out != nil}.as_json.each{|x| x["type"] = "check_out"}].flatten.sort{|x,y| x["date"] <=> y["date"]}
